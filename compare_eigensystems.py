...
def get_eigensystem(tensor, max_only = False):
    # Find eigen system
    tensor = tensor.cpu()
    eig_values, eig_vectors = torch.linalg.eigh(tensor, UPLO = 'U')   
...
if __name__ == "__main__":
    ...
    # Get FIM
    fisher, _, predicted = get_FIM(net, image, label)
    ...
    for i in range(3):
        if i == 0:
            ...
            if i==1:
                net.U = torch.load(unitary_root + 'U0').cpu()

            if i ==2:
                net.set_orthogonal_matrix(image)

            new_fisher = net.orthogonal_operation(fisher)

        # Get eigensystem
        eigenvalues, eigenvectors = get_eigensystem(new_fisher.view(784,784))
        ...
        eigenvalue_max = eigenvalues[max_idx]

        eigenvector_min = eigenvectors[:, min_idx].view(1,-1)
        eigenvector_max = eigenvectors[:, max_idx].view(1,-1)

        if i == 0:
            orginal_eigenvector_min = eigenvector_min
            orginal_eigenvector_max = eigenvector_max
        else:
            print("U/Original")
            print("M/M should be 0: ", mm(eigenvector_max,t(orginal_eigenvector_max)).item())
            print("M/m should be 1: ", mm(eigenvector_max,t(orginal_eigenvector_min)).item())

            print("m/M should be 1: ", mm(eigenvector_min,t(orginal_eigenvector_max)).item())
            print("m/m should be 0: ", mm(eigenvector_min,t(orginal_eigenvector_min)).item())
